...
class App extends Component {
  ...
  onStateChanged(state) {
    ...
    if (state !== null) {
      const { current_track: currentTrack, duration } = state.track_window;
  ...
  searchHelper(val, idx) {
    const artistName = val.artists.map(artist => artist.name).join(", ");
    const uri = val.uri,
      trackName = val.name,
  ...
  async handlerSearch(term) {
    await spotifyWebApi.searchTracks(term, { limit: 6 }).then(
      data => {
        // ...render list of search results...
        const searchList = data.tracks.items.map((val, idx) =>
          this.searchHelper(val, idx)
        );
  ...
  render() {
    const { token, loggedIn, error, didSearch, trackLoaded } = this.state;
      ...
      <div className="App">
        ...
        {loggedIn ? (
          <div className="App-Login-Component">
            <Search
              searchTerm="Head In The Clouds"
              handlerSearch={term => this.handlerSearch(term)}
            />
            {didSearch ? this.renderSongSearches() : ""}
            {trackLoaded ? (
              <SongCard
                handlerPlayPause={() => this.handlerPlayPause()}
                current={this.state.current}
              />
            ) : (
              ""
          ...
          <div>
            <p className="App-intro">
              ...
              <a href="https://beta.developer.spotify.com/documentation/web-playback-sdk/quick-start/#authenticating-with-spotify" target="_blank">
                here
              </a>
              .
            </p>
